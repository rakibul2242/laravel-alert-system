<div class="max-w-md mx-auto space-y-6">
    <form wire:submit.prevent="sendMail" class="space-y-6">
        @if ($successMessage)
            <div class="text-green-800 bg-green-100 border border-green-300 rounded px-4 py-3" role="alert">
                {{ $successMessage }}
            </div>
        @endif
        ...
        <div>
            ...
            <textarea
                ...
                class="w-full border border-gray-300 rounded-md px-4 py-2 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
            ></textarea>
            @error('content') 
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p> 
            @enderror
